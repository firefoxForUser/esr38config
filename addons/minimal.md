...
* В стандартный настройках:
	...
	* Отключить защиту от вредоносных доменов и мошеннических доменов. Сюда входит и функция отправки хэша (уникального идентификатора) каждого загружаемого файла в сервисы гугл для анализа на вредоносность. Дополнительно можно установить значения на странице ```about:config```: <br> ```browser.safebrowsing.downloads.enabled = false```, ```browser.safebrowsing.downloads.remote.enabled = false```
