...
#### 0. Минимум дополнений и настроек <br>
Для тех, кто хочет быстро настроить Firefox для нормального использования. <br>
Минимум без которого нормально использовать Firefox нельзя: [тут](addons/minimal.md)
...
Можно создать файл ```mozilla.cfg``` в профиле Firefox и прописать там скрит. Подробнее [тут](https://developer.mozilla.org/en-US/Firefox/Enterprise_deployment#Configuration) и [тут](https://mike.kaply.com/2012/03/16/customizing-firefox-autoconfig-files/) <br>
...
Полноценные темы. Если темы не подходят, то смотрите в разделе версий более старые. Если цвета тем не совместимы с цветами фонов и надписей, то можно править через CSS или [Tab Mix Plus](https://addons.mozilla.org/en-us/firefox/addon/tab-mix-plus)
...
[Dictionary Switcher](https://addons.mozilla.org/en-us/firefox/addon/dictionary-switcher) - дополнение для быстрого переключения словарей <br>
...
**Несколько комманд в качестве примера:** <br>
```firefox -P``` - запускает менеджер через кото можно создать профиль или выбрать существующий для запуска. <br>
```firefox -no-remote -profile c:\my_profile_folder_name``` запуск созданного профиля с указанием каталога в котором он находится. (в Windows системах эта команда ещё и создаёт новый профиль) <br>
...
Дополнительно в настройках Firefox в разделе ```Содержимое``` (```Content```) можно выбрать список языков, который Firefox передат сайту, чтобы сайт понял на каком языке пользователь предпочитает видеть сайт (те сайты которые поддерживают многоязычность). <br>
...
#### 19. Дополнительные страницы функций Firefox (about:, .xul, панели)

Будет описано.

#### 20. [Синхронизация](https://support.mozilla.org/en-US/products/firefox/sync)
...
#### 22. Привязка программ к типу открываемого файла (MIME-type, чем открыть файл с сайта)

Будет описано.

#### 23. Управление сертификатам сайтов

Будет описано.

#### 24. Ключевые слова для поисковых запросов и закладок

Кликнув по строке для ввода текста правой кнопкой мыши (ПКМ) можно выбрать "создание ключевого слова для поиска". По сути это закладка при клике на которую вы перейдете на сервис поиска. Но если набрать заданное для неё ключевое слово, а затем запрос в **строке навигации** (строке адреса, а не поиска), то ваш запрос подставится в качестве параметра. <br>
```
google мой_запрос
```
При этом можно создать любую закладку и дать ей ключевое слово, при использовании ключевого слова запрос после него подставится вместо параметра ```%s``` в закладке: <br>
```
https://google.ru/%s/
```

**Ссылки:** <br>
[официальная страница](http://www-archive.mozilla.org/docs/end-user/keywords.html) <br>
[описание на kb.mozillazine.org](http://kb.mozillazine.org/Using_keyword_searches)
